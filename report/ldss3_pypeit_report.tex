...
performance and \cref{sec:results} example reductions, and \cref{sec:issues}
lists the known limitations. The scripts that produce every
...
It must be stressed that the frame type cannot be determined from the header
alone. LDSS3-C writes no keyword that identifies an exposure as an arc, a flat
or a science frame: \code{EXPTYPE} takes only the values \code{Object},
\code{Flat}, \code{Bias} and \code{Dark}, and, as noted above, arcs and flats
are written with either of the first two. What the pipeline does is therefore a
naive inference from the free text of the \code{OBJECT} card, which the observer
is at liberty to write as they please. It reproduces the naming conventions we
have encountered in the archive (\cref{tab:typing}) and will fail, silently, on
any convention it has not seen. \textbf{The classification must always be
checked against the observing log before the data are reduced.} The
\code{.pypeit} file lists one row per exposure together with the type assigned
to it, and editing that column is the supported way of correcting it.

\begin{table}[t]
  \centering\small
  \caption{Frame classification inferred from the \code{OBJECT} card. These are
  heuristics, not header keywords, and must be verified against the observing
  log.}
  \label{tab:typing}
  \begin{tabular}{lll}
    \toprule
...
the second removed by hand in the input file, which is the supported way of
...
studied, but should be considered in mask design.
...
\input{tables/readnoise}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.62\textwidth]{figures/readnoise}
...
The dark current, measured from dark exposures of \darkExptimes~s bracketed by biases
...
\Cref{fig:science} shows, for each data set in which a source was detected, the
sky-subtracted two-dimensional spectrum and the optimally extracted
...
$S/N\sim1$ per pixel in a single exposure, and are shown in 8-pixel bins. The
VPH-Blue longslit and the VPH-Red mask CDFS01 yielded no detected source and are
not considered here; their arc and flat calibration is reported in
\cref{sec:wavecal} as for the other data sets.
...
noise-limited even on the brightest lines. In the multi-slit mask ATM3a2\_1 it is
\chiFaintBlueMOS{} and \chiBrightBlueMOS{}, and the distribution on bright lines
has extended tails: sky-line residuals exceed the noise by
\SIrange{10}{20}{\percent}. These
...
\code{frametype} column against the observing log --- the types are inferred
from the text of the \code{OBJECT} card and cannot be taken on trust
(\cref{sec:typing}) --- and reduce:
\begin{lstlisting}
